...
class BEVDepthLightningModel(LightningModule):
    ...
    def training_step(self, batch):
        ...
        planar_loss = self.compute_planar_loss(depth_preds)

        total_loss = detection_loss + depth_loss + self.lambda_normal*planar_loss['normal'] + self.lambda_linear*planar_loss['linear']

        self.log('detection_loss', detection_loss)
        self.log('depth_loss', depth_loss)
        self.log('planar_loss_normal', planar_loss['normal'])
        self.log('planar_loss_linear', planar_loss['linear'])
    ...
    ### 3D-ASAP prior: Planar Consistency Regularizaiton 추가 ###

    def compute_normal_loss(self, depth):
        # depth: [B, D, H, W] or [B, 1, H, W]
        # B: 배치 크기 / D: depth 채널 수(depth_bins의 개수) / H, W: 이미지 크기
        ...
        edge_weight = torch.exp(-10 * edge_strength)

        # mask reshape
        weight_x = edge_weight[:, :, :, 1:] * edge_weight[:, :, :, :-1]
        weight_y = edge_weight[:, :, 1:, :] * edge_weight[:, :, :-1, :]

        # cosine similarity
        sim_x = (nx[:, :, :, 1:] * nx[:, :, :, :-1])
        sim_y = (ny[:, :, 1:, :] * ny[:, :, :-1, :])

        cos_sim_x = (sim_x * weight_x).mean()
        cos_sim_y = (sim_y * weight_y).mean()
        
        return 1 - 0.5 * (cos_sim_x + cos_sim_y)
    
    def compute_linearity_loss(self, depth, alpha=10.0):
        A = depth[:, :, :-2, :-2]
        B = depth[:, :, 1:-1, 1:-1]
        C = depth[:, :, 2:, 2:]
        diff1 = B - A
        ...
        gx = F.pad(depth[:, :, :, 1:] - depth[:, :, :, :-1], (0, 1))
        gy = F.pad(depth[:, :, 1:, :] - depth[:, :, :-1, :], (0, 0, 0, 1))
        edge_strength = torch.sqrt(gx**2 + gy**2 + 1e-6)

        edge_weight = edge_strength[:, :, 1:-1, 1:-1]
        edge_weight = torch.exp(-alpha * edge_weight) 

        loss = F.l1_loss(diff1 * edge_weight, diff2 * edge_weight)

        return loss
        
    def compute_planar_loss(self, depth_pred):
        normal_loss = self.compute_normal_loss(depth_pred)
        linear_loss = self.compute_linearity_loss(depth_pred)
        
        return {'normal': normal_loss, 'linear': linear_loss}
